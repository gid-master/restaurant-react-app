...
const ProductAdditional:React.FC<ProductAdditionalProps> = (props:ProductAdditionalProps) =>{
  ...
  const disableIncrement = ():boolean => {
    const value: number = props.additionals.reduce(
      (total:number, { quantity }) => total + quantity,
      0,
    );
      ...
      {props.additionals.map((additional:IProductAdditional, index:number) => 
        <div key={additional.id}>
          <div className="control">
            <div className="item">
              <span className="price">{ formatCurrency(additional.price) }</span>
              <span className="ingredient">{ additional.name }</span>
          ...
          {index < props.additionals.length - 1 && <div className="separator" />}
        </div>,  
      )}
    </div>
  )
