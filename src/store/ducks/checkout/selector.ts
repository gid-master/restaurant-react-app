...
export const getCart = (state:ApplicationState) =>  state.checkout.cart.map((cart:ICart) => ({
  ...cart,
  product: {
    ...cart.product,
  ...
  (cart:ICart[]) => {
    ...
    const totalAdditionals = cart.reduce(
      (totalProduct: number, { product }) =>
        totalProduct + product.additionals.reduce(
          (totalAdditional: number, { quantity, price }) => totalAdditional + quantity * price * product.quantity, 0),
      0,
    );
    ...
    const quantityAdditionals = cart.reduce(
      (totalProduct: number, { product }) =>
        totalProduct + product.additionals.reduce(
          (totalAdditional: number, { quantity }) => totalAdditional + quantity * product.quantity, 0),
      0,
    );
